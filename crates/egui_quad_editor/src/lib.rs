...
//!
//! Look at [`QuadEditor`] to get started.
use std::collections::BTreeMap;
use std::ops::Range;

use egui::{Context, Label, RichText, ScrollArea, Sense, TextEdit, Ui, Vec2, Widget, Window};

use crate::option_data::{BetweenFrameData, QuadEditorOptions};
...
pub struct QuadEditor {
    ...
    /// A collection of options relevant for the `QuadEditor` window.
    /// Can optionally be serialized/deserialized with `serde`
    pub options: QuadEditorOptions,
...
impl QuadEditor {
    /// Create the QuadEditor, which should be kept in memory between frames.
    ///
    ...
    /// let mut quad_editor = QuadEditor::new().with_address_range("Memory", 0..0xFF);
    ///
    /// // Show a read-only window
    /// quad_editor.window_ui_read_only(&ctx, &mut is_open, &mut memory_base, |mem, addr| mem[addr].into());
    /// ```
    pub fn new() -> Self {
        QuadEditor {
    ...
    fn draw_editor_contents_impl<T: ?Sized>(
        ...
        let QuadEditorOptions {
            show_ascii,
            column_count,
            address_text_colour,
            highlight_text_colour,
            selected_address_range,
            quad_editor_address_text_style,
            ..
        } = self.options.clone();
                ...
                .show(ui, |ui| {
                    ...
                    for start_row in line_range.clone() {
                        ...
                        let start_text = RichText::new(format!("0x{:01$X}:", start_address, address_characters))
                            ...
                            .text_style(quad_editor_address_text_style.clone());
                        ...
                        && matches!(frame_data.selected_edit_address, Some(address) if address == memory_address)
                    {
                        ...
                        let response = ui.add(
                            TextEdit::singleline(&mut frame_data.selected_edit_address_string)
                                ...
                                .font(options.quad_editor_text_style.clone())
                        ...
                        // Read-only values.
                        let mut text = RichText::new(label_text).text_style(options.quad_editor_text_style.clone());
        ...
        ui.horizontal(|ui| {
            ...
            ui.horizontal(|ui| {
                for i in 0..options.column_count {
                    ...
                    let mut text = RichText::new(character).text_style(options.quad_editor_ascii_text_style.clone());
    ...
    fn get_line_height(&self, ui: &mut Ui) -> f32 {
        let address_size = ui.text_style_height(&self.options.quad_editor_address_text_style);
        let body_size = ui.text_style_height(&self.options.quad_editor_text_style);
        let ascii_size = ui.text_style_height(&self.options.quad_editor_ascii_text_style);
    ...
    #[inline]
    #[must_use]
    pub fn with_options(mut self, options: QuadEditorOptions) -> Self {
    ...
    pub fn set_options(&mut self, options: QuadEditorOptions) {
        self.options = options;
    }
}

impl Default for QuadEditor {
    fn default() -> Self {
        QuadEditor::new()
    }
}
